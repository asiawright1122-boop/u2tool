...
chartComponents.forEach(file => {
  ...
  if (chartType) {
    ...
    // 3. 修复Expected 1 arguments, but got 2错误
    content = content.replace(
      /alert\(t\('csvImportSuccess'\), \{ count: ([^)]*) \}\)\)/g,
      (match, count) => `alert(t('csvImportSuccess').replace('{count}', String(${count})))`
    );
    
    // 4. 修复重复的as const断言 - 使用更精确的模式
    content = content.replace(
      /'bold' as const as const/g,
      "'bold' as const"
    );
    content = content.replace(
      /'normal' as const as const/g,
      "'normal' as const"
    );

<script lang="ts">
  ...
  function t(key: string): string {
    const scope = translations['tools']['json-to-swift'] as Record<string, unknown> || {};
    const keys = key.split('.');
    let value: unknown = scope;
    for (const k of keys) { value = (value as Record<string, unknown>)?.[k]; }
    return typeof value === 'string' ? value : `MISSING: tools.json-to-swift.${key}`;
  }
  ...
  function copyToClipboard() {
    ...
    <div class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label for="json-to-swift-field-6" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('structName')}</label>
          <input type="text" bind:value={structName}
            class="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white" id="json-to-swift-field-6" />
        </div>
        <div class="flex items-center gap-4">
          <label class="flex items-center gap-2 text-gray-600 dark:text-gray-300">
            <input type="checkbox" bind:checked={useCodable}
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <label for="json-to-swift-field-5" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('input')}</label>
          <textarea bind:value={input}
            ...
            placeholder={t('inputPlaceholder')} id="json-to-swift-field-5"></textarea>
        </div>
        <div>
          <label for="json-to-swift-field-4" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('output')}</label>
          <textarea value={output} readOnly
            ...
            placeholder={t('outputPlaceholder')} id="json-to-swift-field-4"></textarea>
        </div>
      </div>

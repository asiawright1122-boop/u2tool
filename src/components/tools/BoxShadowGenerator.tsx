      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-medium text-white">{t('shadowLayers')}</h3>
            <button
              onClick={addShadow}
          ...
          {shadows.map((shadow, index) => (
            <div key={shadow.id} className="bg-gray-700 rounded-lg p-4 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-gray-300 font-medium">{t('layer', { index: index + 1 })}</span>
                {shadows.length > 1 && (
                  <button onClick={() => removeShadow(shadow.id)} className="text-red-400 hover:text-red-300 text-sm">
                    {t('remove')}
                  </button>
                )}
              </div>
              ...
              <div className="flex gap-4 items-center">
                <div className="flex items-center gap-2">
                  <label className="text-xs text-gray-300">{t('color')}</label>
                  <input type="color" value={shadow.color}
                    onChange={(e) => updateShadow(shadow.id, 'color', e.target.value)}
                    className="w-8 h-8 rounded cursor-pointer" />
                </div>
                <div className="flex-1">
                  <label className="block text-xs text-gray-300 mb-1">{t('opacity')}: {shadow.opacity}%</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-2">{t('borderRadius')}: {boxRadius}px</label>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="bg-gray-700 rounded-lg p-4 font-mono text-sm text-green-400 break-all">

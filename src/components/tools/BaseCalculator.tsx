  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-4 items-center">
        <label className="text-sm text-gray-600 dark:text-gray-300">{t('base')}:</label>
        <select
          value={base}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div>
          ...
          <input
            ...
            className="w-full bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white font-mono focus:outline-none focus:border-blue-500"
          />
        </div>
      ...
      {result && (
        <div className="p-4 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg">
          <div className="text-center text-lg font-mono text-gray-700 dark:text-gray-300 mb-4">
            {num1} {operationSymbols[operation]} {num2} = <span className="text-green-600 dark:text-green-400">{result.formatted}</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div className="text-center">
              <div className="text-gray-500 dark:text-gray-400">{t('binary')}</div>
              <div className="font-mono text-gray-900 dark:text-white">{formatNumber(result.decimal, 2)}</div>
            </div>
            <div className="text-center">
              <div className="text-gray-500 dark:text-gray-400">{t('octal')}</div>
              <div className="font-mono text-gray-900 dark:text-white">{formatNumber(result.decimal, 8)}</div>
            </div>
            <div className="text-center">
              <div className="text-gray-500 dark:text-gray-400">{t('decimal')}</div>
              <div className="font-mono text-gray-900 dark:text-white">{result.decimal}</div>
            </div>
            <div className="text-center">
              <div className="text-gray-500 dark:text-gray-400">{t('hex')}</div>
              <div className="font-mono text-gray-900 dark:text-white">{formatNumber(result.decimal, 16)}</div>
            </div>
          </div>
        </div>
      )}

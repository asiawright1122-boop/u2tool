    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          <label className="text-sm font-medium text-gray-300">{t('urls')}</label>
          <button onClick={addUrl} className="px-3 py-1 bg-green-600 hover:bg-green-700 rounded text-sm">
            {t('addUrl')}
          </button>
        </div>
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {urls.map((url, index) => (
            <div key={index} className="grid grid-cols-12 gap-2 bg-gray-700 rounded-lg p-2">
              <input type="text" value={url.loc} onChange={(e) => updateUrl(index, 'loc', e.target.value)}
                className="col-span-4 px-2 py-1 bg-gray-600 border border-gray-500 rounded text-white text-sm"
                placeholder="/path" />
              <input type="date" value={url.lastmod} onChange={(e) => updateUrl(index, 'lastmod', e.target.value)}
                className="col-span-2 px-2 py-1 bg-gray-600 border border-gray-500 rounded text-white text-sm" />
              ...
              <button onClick={() => removeUrl(index)}
                className="col-span-2 px-2 py-1 bg-red-600 hover:bg-red-700 rounded text-sm">{t('remove')}</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-4">
        <button onClick={generate} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg font-medium transition-colors">
          {t('generate')}
        </button>
      ...
      {output && (
        <div>
          ...
          <pre className="bg-gray-700 rounded-lg p-4 font-mono text-sm text-green-400 overflow-x-auto max-h-64">
            {output}
          </pre>
        </div>
      )}

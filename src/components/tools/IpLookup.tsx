    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-gray-700 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
        />
        <button
      ...
      {info && !loading && (
        <div className="bg-gray-700 rounded-lg p-4 space-y-3">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <span className="text-gray-300">{t('ipAddress')}:</span>
              <span className="ml-2 text-white font-mono">{info.ip}</span>
            </div>
            <div>
              <span className="text-gray-300">{t('country')}:</span>
              <span className="ml-2 text-white">{info.country || '-'}</span>
            </div>
            <div>
              <span className="text-gray-300">{t('region')}:</span>
              <span className="ml-2 text-white">{info.region || '-'}</span>
            </div>
            <div>
              <span className="text-gray-300">{t('city')}:</span>
              <span className="ml-2 text-white">{info.city || '-'}</span>
            </div>
            <div>
              <span className="text-gray-300">{t('isp')}:</span>
              <span className="ml-2 text-white">{info.isp || '-'}</span>
            </div>
            <div>
              <span className="text-gray-300">{t('timezone')}:</span>
              <span className="ml-2 text-white">{info.timezone || '-'}</span>
            </div>
            {info.lat && info.lon && (

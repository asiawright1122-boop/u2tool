<script lang="ts">
  ...
  function t(key: string): string {
    const scope = translations['tools']['audio-to-base64'] as Record<string, unknown> || {};
    const keys = key.split('.');
    let value: unknown = scope;
    for (const k of keys) { value = (value as Record<string, unknown>)?.[k]; }
    return typeof value === 'string' ? value : `MISSING: tools.audio-to-base64.${key}`;
  }
    ...
    <div class="space-y-6">
      ...
      >
        ...
        <label for="audio-input" class="cursor-pointer">
          <div class="text-4xl mb-4"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></div>
          <p class="text-gray-600 dark:text-gray-300 mb-2">{t('dropzone')}</p>
          <p class="text-sm text-gray-600 dark:text-gray-300">{t('acceptedFormats')}</p>
        </label>
      </div>
...
<div class="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg space-y-3">
          ...
          {#if audioUrl}
<audio bind:this={audioRef} controls class="w-full">
...
{/if}
        ...
        <textarea
          ...
          class="w-full h-48 p-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-mono text-xs"></textarea>
      </div>

      <div class="p-4 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
        <h3 class="font-medium text-blue-800 dark:text-blue-300 mb-2">{t('supportedFormats')}</h3>
        <p class="text-sm text-blue-700 dark:text-blue-400">MP3, WAV, OGG, WebM, AAC, FLAC, M4A</p>
      </div>
    </div>

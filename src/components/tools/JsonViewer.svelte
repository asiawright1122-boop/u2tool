<script lang="ts">
  ...
  function expandAll() {
    ...
    <div class="space-y-6">
      <div>
        <label for="json-viewer-field-2" class="tool-label">{t('input')}</label>
        <textarea bind:value={input}
          ...
          placeholder={t('inputPlaceholder')} id="json-viewer-field-2"></textarea>
      </div>

      <div class="flex gap-4">
        <button onclick={parse} class="px-6 py-2 bg-amber-600 hover:bg-amber-700 rounded-lg font-medium transition-colors">
          {t('parseView')}
        </button>

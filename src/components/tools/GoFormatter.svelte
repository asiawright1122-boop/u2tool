  ...
  async function copyOutput() {
    ...
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          <label for="go-formatter-field-4" class="block text-sm text-gray-600 dark:text-gray-300 mb-2">{tg('input')}</label>
          <textarea
            bind:value={input}
            ...
            placeholder={t('inputPlaceholder')} id="go-formatter-field-4"></textarea>
        </div>
        <div>
          <label for="go-formatter-field-3" class="block text-sm text-gray-600 dark:text-gray-300 mb-2">{tg('output')}</label>
          <textarea
            value={output}
            readOnly
            class="w-full h-80 bg-gray-100 dark:bg-gray-700 rounded-lg px-4 py-3 text-gray-900 dark:text-white font-mono text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none resize-none" id="go-formatter-field-3"></textarea>
        </div>
      </div>
    </div>

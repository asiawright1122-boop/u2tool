<script lang="ts">
  ...
  function t(key: string): string {
    ...
    const scope = tools['ai-text-humanizer'] as Record<string, unknown> || {};
  ...
  function humanize() {
    ...
    let result = input;
    const patterns = (AI_PATTERNS as Record<string, [string | RegExp, string[]][]>)[intensity];

    patterns.forEach(([pattern, replacements]: [string | RegExp, string[]]) => {
    ...
    if (addFillers) {
      ...
      result = sentences.map((sentence, i) => {
        if (i > 0 && Math.random() > 0.7) {
          const filler = FILLER_WORDS[Math.floor(Math.random() * FILLER_WORDS.length)] as string;
    ...
    <div class="space-y-4">
      <div class="flex flex-wrap gap-6 items-center glass-card p-6 !rounded-2xl">
        <div class="flex items-center gap-3">
          <label class="tool-label !mb-0" for="intensity-select">{t('intensity')}</label>
          <select
            id="intensity-select"
            value={intensity}
            onchange={(e) => intensity = (e.target as HTMLSelectElement).value as Intensity}
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-3">
          <label class="tool-label" for="humanizer-input">{tg('input')}</label>
          <textarea
            id="humanizer-input"
            bind:value={input}
        ...
        <div class="space-y-3">
          <label class="tool-label" for="humanizer-output">{tg('output')}</label>
          <textarea
            id="humanizer-output"
            value={output}
            readOnly

    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4 items-center">
        <div className="flex items-center gap-2">
          <label className="text-sm text-gray-600 dark:text-gray-300">{t('inputMode')}:</label>
          <select
            value={inputMode}
            onChange={(e) => {
      ...
      {inputMode === 'text' ? (
        <div className="space-y-2">
          ...
          <textarea
            ...
            className="w-full h-40 p-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 font-mono"
          />
        </div>
      ) : (
        <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-300">{t('crcHex')}</div>
            {crcHex && (
              <button
          ...
          <div className="mt-1 font-mono text-lg text-gray-900 dark:text-gray-100">{crcHex ? `0x${crcHex}` : '-'}</div>
        </div>

        <div className="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-300">{t('crcDec')}</div>
            {crcDec && (
              <button
          ...
          <div className="mt-1 font-mono text-lg text-gray-900 dark:text-gray-100">{crcDec || '-'}</div>
        </div>
      </div>

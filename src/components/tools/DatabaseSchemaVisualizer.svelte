...
<div  class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="bg-amber-600 text-white px-4 py-2 font-medium">{table.name}</div>
...
<div  class="px-4 py-2 flex items-center justify-between">
                      <div class="flex items-center gap-2">
                        ...
                        {#if col.fk}<span class="text-amber-500" title={`FK: ${col.fk}`}><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></span>{/if}
                ...
                {#each tables as table}
                  {#each table.columns.filter(c => c.fk) as col}
                    <div class="flex items-center gap-2 text-sm">
                      <span class="font-mono text-amber-600 dark:text-amber-400">{table.name}.{col.name}</span>

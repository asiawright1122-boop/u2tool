<script lang="ts">
  ...
  function t(key: string): string {
    const scope =
      (translationsTyped["tools"]?.["radar-chart-generator"] as Record<
        string,
        unknown
      >) || {};
    ...
    return typeof value === "string"
      ? value
      : `MISSING: tools.radar-chart-generator.${key}`;
  }
  ...
  function addIndicator() {
    ...
    indicators = [
      ...indicators,
      { id: newId, name: `${t("indicator")}${newIndex}`, max: 100 },
    ];
    // 为每个系列添加新数据点
    series = series.map((s) => ({
      ...s,
  ...
  function loadSampleData() {
    ...
    indicators = [
      ...
      },
      {
        ...
        name: t("sampleData.support"),
        max: 100,
      },
      {
        id: `${baseId}-${idCounter + 4}`,
        name: t("sampleData.develop"),
        max: 100,
      },
      {
        id: `${baseId}-${idCounter + 5}`,
        name: t("sampleData.marketing"),
        max: 100,
      },
    ];
    series = [
      {
        id: `${baseId}-${idCounter + 6}`,
        name: t("sampleData.budget"),
        values: [80, 90, 70, 85, 75, 88],
      },
      {
        id: `${baseId}-${idCounter + 7}`,
        name: t("sampleData.actual"),
        values: [70, 85, 80, 75, 90, 82],
      },
    ];
  ...
  function parseRadarCSV(csvText: string): ParsedRadarData | null {
    ...
    const indicators: { name: string; max: number }[] = indicatorNames.map(
      (name) => ({
        name,
        max: 100,
      }),
  ...
  function handleCsvImport(event: Event) {
    ...
    reader.onload = (e) => {
      ...
      if (
        ...
        series = newSeries;
        idCounter =
          idCounter + parsed.indicators.length + parsed.seriesData.length;
        alert(
          t("csvImportSuccess").replace(
            "{count}",
            String(parsed.indicators.length),
          ),
        );
      } else {
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label
                for={t("colorTheme")}
                class="block text-sm font-medium mb-1">{t("colorTheme")}</label
              >
              <select
                value={colorTheme}
                onchange={(e) =>
            ...
            <div>
              <label for={t("shape")} class="block text-sm font-medium mb-1"
                >{t("shape")}</label
              >
              <select
                value={shape}
                onchange={(e) =>
                  (shape = (e.target as HTMLInputElement).value as
                    | "polygon"
                    | "circle")}
                class="tool-input"
              >
          ...
          <div>
            <label
              for="{t('fillOpacity')}: {fillOpacity}"
              class="block text-sm font-medium mb-1"
              >{t("fillOpacity")}: {fillOpacity}</label
            >
            <input
              type="range"
              min="0"

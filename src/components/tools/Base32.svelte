  ...
  async function copy() {
    ...
    <div class="space-y-4">
      <div class="flex gap-2">
        <button onclick={() => mode = 'encode'} class={`px-4 py-2 rounded text-white ${mode === 'encode' ? 'bg-amber-600' : 'bg-gray-500 dark:bg-gray-700'}`}>{t('encode')}</button>
        <button onclick={() => mode = 'decode'} class={`px-4 py-2 rounded text-white ${mode === 'decode' ? 'bg-amber-600' : 'bg-gray-500 dark:bg-gray-700'}`}>{t('decode')}</button>
      </div>
      <div>

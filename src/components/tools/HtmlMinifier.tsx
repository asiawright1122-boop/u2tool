    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <textarea value={input} onChange={(e) => setInput(e.target.value)} className="w-full h-64 p-3 bg-gray-800 border border-gray-700 rounded-lg font-mono text-sm" placeholder="<html>..." />
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">{t('output')}</label>
          <textarea value={output} readOnly className="w-full h-64 p-3 bg-gray-800 border border-gray-700 rounded-lg font-mono text-sm" />
        </div>
      </div>
    </div>
  );

  ...
  function handleStyleChange(style: 'dataclass' | 'pydantic' | 'typeddict' | 'plain') {
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
          <label for="json-to-python-field-6" class="block text-sm text-gray-600 dark:text-gray-300 mb-1">{t('className')}</label>
          <input
            type="text"
            value={options.className}
            onchange={(e) => options = { ...options, className: e.target.value }}
            class="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm text-gray-900 dark:text-white" id="json-to-python-field-6" />
        </div>
        <div>
          <label for="json-to-python-field-5" class="block text-sm text-gray-600 dark:text-gray-300 mb-1">{t('style')}</label>
          <select
            ...
            class="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm text-gray-900 dark:text-white" id="json-to-python-field-5">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div>
          <div class="flex justify-between items-center mb-2">
            <div class="text-sm font-medium text-gray-600 dark:text-gray-300">Python</div>
            <button
              onclick={copyOutput}
              disabled={!output}

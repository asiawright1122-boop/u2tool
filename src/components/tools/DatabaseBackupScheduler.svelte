  ...
  function handleCopy(text: string, key: string) {
    ...
    <div class="space-y-6">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <label for="database-backup-scheduler-field-4" class="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t('database')}</label>
          <select value={config.database} onchange={(e) => updateConfig('database', (e.currentTarget as HTMLSelectElement).value as BackupConfig['database'])}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" id="database-backup-scheduler-field-4">
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          <div class="tool-label">{t('backupScript')}</div>
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          <div class="tool-label">{t('crontabEntry')}</div>

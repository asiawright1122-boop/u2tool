<script lang="ts">
  ...
  const t = createToolTranslator(translations, 'grouped-bar-chart-generator');
  ...
  function handleCsvImport(event: Event) {
    ...
    <div class="space-y-4">
      <div class="flex flex-wrap gap-2">
        ...
        <button onclick={clearData} class="btn-secondary"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg> {t('clear')}</button>
      </div>
...
<tr  class="border-b border-gray-100 dark:border-gray-800 last:border-b-0">
                      <td class="py-2 px-2 min-w-[120px]">
                        <input type="text" value={categories[catIndex]} oninput={(e) => { categories[catIndex] = (e.currentTarget as HTMLInputElement).value; categories = [...categories]; }} class="w-full min-w-[100px] px-2 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded text-gray-900 dark:text-gray-100 text-sm" />
                      </td>
                      {#each series as s, seriesIndex (seriesIndex)}
<td  class="py-2 px-2">
                          <input type="number" value={series[seriesIndex].values[catIndex]} oninput={(e) => { series[seriesIndex].values[catIndex] = Number((e.currentTarget as HTMLInputElement).value) || 0 ; series = [...series]; }} class="w-full px-2 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded text-gray-900 dark:text-gray-100 text-sm" />
                        </td>
{/each}
                      <td class="py-2 px-2">

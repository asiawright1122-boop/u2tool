<script lang="ts">
  ...
  function t(key: string): string {
    const scope = (translations['tools']['grouped-bar-chart-generator'] as Record<string, unknown>) || {};
    const keys = key.split('.');
    let value: unknown = scope;
    for (const k of keys) { value = (value as Record<string, unknown>)?.[k]; }
    return typeof value === 'string' ? value : `MISSING: tools.grouped-bar-chart-generator.${key}`;
  }

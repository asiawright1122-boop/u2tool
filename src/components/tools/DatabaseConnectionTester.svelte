  ...
  const codeSnippet = generateCodeSnippet(config);
    ...
    <div class="space-y-6">
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div>
          <label for="database-connection-tester-field-4" class="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t('database-connection-tester.databaseType')}</label>
          <select value={config.type} onchange={(e) => updateConfig('type', e.target.value as ConnectionConfig['type'])}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" id="database-connection-tester-field-4">
        ...
        <div>
          ...
          <input type="text" id="db-port" name="port" bind:value={config.port}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" />
        </div>
        <div>
          <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1" for="db-name">{t('database-connection-tester.database')}</label>
          <input type="text" id="db-name" name="database" bind:value={config.database}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="password" id="db-password" name="password" bind:value={config.password}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" />
        </div>
      </div>
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          <div class="tool-label">{t('database-connection-tester.connectionString')}</div>
          <button onclick={() => handleCopy(connectionString, 'conn')} class="text-sm text-amber-600 hover:text-amber-700 dark:text-amber-400">
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          <div class="tool-label">{t('database-connection-tester.codeSnippet')}</div>
          <button onclick={() => handleCopy(codeSnippet, 'code')} class="text-sm text-amber-600 hover:text-amber-700 dark:text-amber-400">

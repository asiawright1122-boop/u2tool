    ...
    <div className="space-y-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-2">
            ...
            <input type="text" value={color} onChange={(e) => setColor(e.target.value)}
              className="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white font-mono focus:outline-none focus:border-blue-500" />
          </div>
        </div>
      ...
      {results.length > 0 && (
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-300">{t('closestColors')}</label>
          {results.map((r, i) => (
            <div key={i} className="flex items-center gap-3 p-3 bg-gray-700 rounded-lg">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-400">{r.hex}</div>
              </div>
              <div className="text-sm text-gray-400">{t('distance')}: {r.distance.toFixed(1)}</div>
            </div>
          ))}
        </div>
      )}

...
export default function BarcodeGenerator() {
  ...
  const renderBarcode = () => {
    ...
    return (
      <svg viewBox={`0 0 ${bars.length * 2} 100`} className="w-full h-24 bg-gray-800">
        {bars.split('').map((b, i) => (
  ...
  return (
    <div className="space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-2">{t('barcode.text')}</label>
          <input type="text" value={text} onChange={(e) => setText(e.target.value)} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg" />
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">{t('barcode.format')}</label>
          <select value={format} onChange={(e) => setFormat(e.target.value as typeof format)} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg">
      ...
      <div className="bg-gray-800 rounded-lg p-4">
        {renderBarcode()}
        <p className="text-center text-black mt-2 font-mono">{text}</p>
      </div>
    </div>
  );
}

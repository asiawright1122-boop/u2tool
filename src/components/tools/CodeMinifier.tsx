...
export default function CodeMinifier() {
  ...
  return (
    <div className="space-y-4">
      {/* Code Type Selection */}
      <div>
        <label className="block text-sm font-medium mb-2">{tc('codeType')}</label>
        <div className="flex gap-2">
          {(['html', 'css', 'js'] as CodeType[]).map((type) => (
            <button
              key={type}
              onClick={() => setCodeType(type)}
              className={`px-4 py-2 rounded-lg uppercase font-medium ${
                codeType === type ? 'bg-blue-600' : 'bg-gray-800 hover:bg-gray-700'
              }`}
            >
              {type}
            </button>
      ...
      {output && (
        <>
          ...
          <div className="flex gap-4 text-sm">
            <span className="text-gray-300">{tc('original')}: {stats.original} bytes</span>
            <span className="text-gray-300">{tc('minified')}: {stats.minified} bytes</span>
            <span className="text-green-400">{tc('saved')}: {stats.saved}%</span>
          </div>

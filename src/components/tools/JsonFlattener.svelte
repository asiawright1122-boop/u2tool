  ...
  function copyToClipboard() {
    ...
    <div class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label for="json-flattener-field-8" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('mode')}</label>
          <select value={mode} onchange={(e) => mode = e.target.value as 'flatten' | 'unflatten'}
            class="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white" id="json-flattener-field-8">
            <option value="flatten">{t('flatten')}</option>
            <option value="unflatten">{t('unflatten')}</option>
          </select>
        </div>
        <div>
          <label for="json-flattener-field-7" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('delimiter')}</label>
          <input type="text" bind:value={delimiter}
            class="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white" id="json-flattener-field-7" />
        </div>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <label for="json-flattener-field-6" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('input')}</label>
          <textarea bind:value={input}
            ...
            placeholder={t('inputPlaceholder')} id="json-flattener-field-6"></textarea>
        </div>
        <div>
          <label for="json-flattener-field-5" class="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">{t('output')}</label>
          <textarea value={output} readOnly
            ...
            placeholder={t('outputPlaceholder')} id="json-flattener-field-5"></textarea>
        </div>
      </div>

      ...
      {Object.keys(hashes).length > 0 && (
        <div className="space-y-3">
          {Object.entries(hashes).map(([algo, hash]) => (
            <div key={algo} className="p-3 bg-gray-900 border border-gray-700 rounded-lg">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-medium text-blue-400">{algo}</span>
                <button
                  onClick={() => copyHash(algo, hash)}
                  className={`text-xs px-2 py-1 rounded ${
                    copied === algo ? 'bg-green-600' : 'bg-gray-700 hover:bg-gray-600'
                  }`}
                >
                  {copied === algo ? t('copied') : t('copy')}
                </button>
              </div>
              <div className="font-mono text-xs break-all text-gray-300 select-all">
                {hash}
              </div>
            </div>
          ))}

...
export default function GroupedLineChartGenerator() {
  const t = useTranslations('tools.grouped-line-chart-generator');
  const tg = useTranslations('tools');

  const [isInitialized, setIsInitialized] = useState(false);
  const [categories, setCategories] = useState<string[]>(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']);
  ...
  useEffect(() => {
    if (!isInitialized) {
      ...
      setCategories([t('sampleData.jan'), t('sampleData.feb'), t('sampleData.mar'), t('sampleData.apr'), t('sampleData.may'), t('sampleData.jun')]);
      setSeries([
          ...
          <div>
            ...
            <div className="space-y-3 p-4 bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
              ...
              <div>
                ...
                <select value={lineStyle} onChange={(e) => setLineStyle(e.target.value as 'solid' | 'dashed' | 'dotted')} className="tool-input">
              ...
              <div className="flex flex-wrap gap-6 text-sm">
                ...
                <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={smooth} onChange={(e) => setSmooth(e.target.checked)} className="w-4 h-4 accent-blue-500" /><span>{t('smooth')}</span></label>
              </div>
            </div>
          </div>

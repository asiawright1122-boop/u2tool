<script lang="ts">
  ...
  function t(key: string): string {
    const scope = translations['tools']['waterfall-chart-generator'] as Record<string, unknown> || {};
    const keys = key.split('.');
    let value: unknown = scope;
    for (const k of keys) { value = (value as Record<string, unknown>)?.[k]; }
    return typeof value === 'string' ? value : `MISSING: tools.waterfall-chart-generator.${key}`;
  }
...
<tr  class="border-b border-gray-100 dark:border-gray-800 last:border-b-0">
                      <td class="py-2 px-2 min-w-[120px]">
                        <input type="text" value={row.category} onchange={(e) => updateRow(row.id, 'category', e.target.value)} class="w-full min-w-[100px] px-2 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded text-gray-900 dark:text-gray-100 text-sm" />
                      </td>
                      <td class="py-2 px-2 min-w-[100px]">
                        <input type="number" value={row.value} onchange={(e) => updateRow(row.id, 'value', e.target.value)} class="w-full min-w-[80px] px-2 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded text-gray-900 dark:text-gray-100 text-sm" />
                      </td>
                      <td class="py-2 px-2 min-w-[100px]">
                        <select value={row.type} onchange={(e) => updateRow(row.id, 'type', e.target.value)} class="w-full min-w-[80px] px-2 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded text-gray-900 dark:text-gray-100 text-sm">

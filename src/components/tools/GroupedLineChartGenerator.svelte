<script lang="ts">
  ...
  function t(key: string): string {
    const scope = translations['tools']['grouped-line-chart-generator'] as Record<string, unknown> || {};
    const keys = key.split('.');
    let value: unknown = scope;
    for (const k of keys) { value = (value as Record<string, unknown>)?.[k]; }
    return typeof value === 'string' ? value : `MISSING: tools.grouped-line-chart-generator.${key}`;
  }
  ...
  function getChartOption() {
    ...
    return {
      ...
      series: series.map((s, index) => ({
        ...
        data: s.values,
        smooth: smooth,
        lineStyle: { type: lineStyle, color: colors[index % colors.length] },
  ...
  $effect(() => {
    if (!isInitialized) {
      ...
      categories = [t('sampleData.jan'), t('sampleData.feb'), t('sampleData.mar'), t('sampleData.apr'), t('sampleData.may'), t('sampleData.jun')];
      series = [
        { name: t('sampleData.series1'), values: [120, 200, 150, 80, 170, 110] },
        { name: t('sampleData.series2'), values: [90, 150, 180, 120, 140, 160] },
      ];
      isInitialized = true;
    }
  });  onDestroy(() => {
  ...
  function exportChart(format: 'png' | 'svg') {
    ...
    link.download = `grouped-line-chart-${Date.now()}.${format}`;
    link.href = url;
    link.click();
  }
  function loadSampleData() {
    categories = [t('sampleData.jan'), t('sampleData.feb'), t('sampleData.mar'), t('sampleData.apr'), t('sampleData.may'), t('sampleData.jun')];
    series = [
      { name: t('sampleData.sales'), values: [320, 280, 350, 400, 380, 420] },
      { name: t('sampleData.profit'), values: [120, 100, 150, 180, 160, 200] },
      { name: t('sampleData.cost'), values: [200, 180, 200, 220, 220, 220] },
    ];
  ...
  function handleCsvImport(event: Event) {
    ...
    <div class="space-y-4">
      <div class="flex flex-wrap gap-2">
        ...
        <button onclick={() => fileInputRef?.click()} class="btn-secondary">📁 {t('importCsv')}</button>
        ...
        <button onclick={clearData} class="btn-secondary">🗑️ {tg('clear')}</button>
      </div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <div class="space-y-3 p-4 bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
              ...
              <div>
                <label class="block text-sm font-medium mb-1">{t('lineStyle')}</label>
                <select value={lineStyle} onchange={(e) => lineStyle = e.target.value as 'solid' | 'dashed' | 'dotted'} class="tool-input">
              ...
              <div class="flex flex-wrap gap-6 text-sm">
                ...
                <label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" bind:checked={smooth} class="w-4 h-4 accent-blue-500" /><span>{t('smooth')}</span></label>
              </div>
            </div>
          </div>

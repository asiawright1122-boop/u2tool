  ...
  function handleCopy() {
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <label for="database-migration-generator-field-10" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('tableName')}</label>
          <input
            type="text"
            bind:value={tableName}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" id="database-migration-generator-field-10" />
        </div>
        <div>
          <label for="database-migration-generator-field-9" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('database')}</label>
          <select
            value={database}
            onchange={(e) => database = e.target.value as Database}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" id="database-migration-generator-field-9">
        ...
        <div>
          <label for="database-migration-generator-field-8" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('framework')}</label>
          <select
            value={framework}
            onchange={(e) => framework = e.target.value as Framework}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" id="database-migration-generator-field-8">
        ...
        <div>
          <label for="database-migration-generator-field-7" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('action')}</label>
          <select
            value={action}
            onchange={(e) => action = e.target.value as 'create' | 'alter'}
            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" id="database-migration-generator-field-7">
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          <div class="tool-label">{t('columns')}</div>
          <button
            onclick={addColumn}

...
<div class="bg-white/70 dark:bg-zinc-900/90 border border-amber-900/10 dark:border-amber-900/30 rounded-xl p-5 shadow-2xl space-y-4">
  <div class="border-b border-amber-900/10 dark:border-amber-900/20 pb-2">
  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left border-collapse text-xs">
      ...
      <tbody>
        {#each calcState.thirdPlaceRankings as team, i}
          {@const isQualified = i < 8}
          {@const groupTeams = calcState.groupTeamsMap}
          {@const teamGroup = Object.keys(groupTeams).find(k => groupTeams[k].includes(team.id)) || ''}
          
          <!-- Border separating qualified and eliminated -->
          {#if i === 8}
            <tr class="bg-amber-50 dark:bg-amber-950/20 border-y border-amber-900/10 dark:border-amber-900/30">
              <td colspan="8" class="py-1.5 px-3 text-center text-[10px] uppercase tracking-wider font-semibold text-amber-700 dark:text-amber-500/80">
          ...
          <tr class={`border-b border-zinc-200 dark:border-zinc-800/40 hover:bg-slate-50 dark:hover:bg-zinc-800/20 transition-colors ${
            isQualified ? 'bg-amber-500/[0.01] dark:bg-amber-500/[0.02]' : 'opacity-60'
          }`}>
            <!-- Rank # -->
            <td class="py-2 text-center font-semibold">
              <span class={`inline-flex items-center justify-center w-5 h-5 rounded-full ${
                isQualified
                  ...
                  : 'bg-slate-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500'
              }`}>
                {i + 1}
              </span>
            </td>
            ...
            <!-- Group -->
            <td class="py-2 text-center font-bold text-zinc-500 dark:text-zinc-400 font-mono">
              {teamGroup}
            </td>
            ...
            <td class="py-2 font-medium">
              <div class="flex items-center gap-1.5">
                ...
                <span class="text-zinc-700 dark:text-zinc-200 truncate max-w-[120px]" title={t(`teams.${team.id}`)}>
                  {t(`teams.${team.id}`)}
                </span>
              </div>
            </td>
            ...
            <!-- GF -->
            <td class="py-2 text-center text-zinc-500 dark:text-zinc-400 font-mono">{team.gf}</td>

            <!-- Points -->
            <td class={`py-2 text-center font-mono font-bold ${isQualified ? 'text-amber-600 dark:text-amber-400' : 'text-zinc-500 dark:text-zinc-300'}`}>
              {team.points}
            </td>

            <!-- Swap action -->
            <td class="py-2 text-center">
              {#if i < calcState.thirdPlaceRankings.length - 1 && canSwap(team, calcState.thirdPlaceRankings[i + 1])}
                <button
                  onclick={() => calcState.swapThirdPlace(i, i + 1)}

<script lang="ts">
  ...
  function handleTeamClick(matchId: string, teamId: string) {
    ...
    // If already predicted, toggle/remove it
    if (state.predictedWinners[matchId] === teamId) {
      state.predictWinner(matchId, null);
    } else {
      state.predictWinner(matchId, teamId);
    }
  }

  function getDisplayInfo(teamId: string, placeholder: string) {
    if (!teamId || teamId.startsWith('TBA_') || teamId.startsWith('3rd_')) {
      // It's a placeholder, try to localize it or keep it as placeholder text
      // Localize placeholder if available
      return {
        id: '',
...
<div class="w-full overflow-x-auto pb-8 pt-4 select-none scrollbar-thin scrollbar-thumb-amber-900/30 scrollbar-track-transparent">
  <div class="flex gap-8 min-w-[1300px] h-[960px] px-4">
    ...
    <div class="flex flex-col justify-around h-full w-[240px] shrink-0">
      <div class="text-center font-bold text-amber-500 uppercase tracking-widest border-b border-amber-900/20 pb-2 mb-2 shrink-0">
        {t('r32_title')}
      </div>
      {#each r32List as mId}
        {@const match = state.knockoutBracket.matchesMap[mId]}
        {#if match}
          ...
          <div class="bg-zinc-950/80 border border-zinc-800/80 rounded-lg p-2.5 shadow-lg space-y-1.5 hover:border-amber-500/30 transition-all">
            ...
            <div class="space-y-1">
              ...
              <button
                ...
                class={`w-full flex items-center justify-between px-2 py-1.5 rounded transition-all text-left ${
                  home.isTba ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-zinc-800/50'
                } ${match.winner === match.homeTeam ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20 font-semibold' : 'text-zinc-300'}`}
              >
              ...
              <button
                ...
                disabled={away.isTba}
                class={`w-full flex items-center justify-between px-2 py-1.5 rounded transition-all text-left ${
                  away.isTba ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-zinc-800/50'
                } ${match.winner === match.awayTeam ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20 font-semibold' : 'text-zinc-300'}`}
              >
                <div class="flex items-center gap-1.5 truncate">
                  {#if !away.isTba}
    ...
    <div class="flex flex-col justify-around h-full w-[260px] shrink-0">
      ...
      <!-- Final (M104) -->
      {#if state.knockoutBracket.matchesMap['M104']}
        {@const finalMatch = state.knockoutBracket.matchesMap['M104']}
        ...
        <div class="space-y-4">
          <div class="text-xs text-center font-bold text-amber-400 uppercase tracking-wider">{t('final_title')}</div>
          <div class="bg-zinc-950 border-2 border-amber-500/40 rounded-xl p-4 shadow-2xl space-y-3 relative overflow-hidden bg-gradient-to-b from-zinc-900 to-black">
            <div class="absolute top-0 right-0 p-1.5 bg-amber-500/10 border-b border-l border-amber-500/20 rounded-bl text-[8px] font-mono text-amber-400 font-bold">
              {finalMatch.id}
            </div>
            ...
            <div class="space-y-2">
              <button
                ...
                class={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all text-left ${
                  home.isTba ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-zinc-800/80'
                } ${finalMatch.winner === finalMatch.homeTeam ? 'bg-amber-500/20 text-amber-400 border border-amber-500 font-bold' : 'text-zinc-300'}`}
              >
                ...
                {#if finalMatch.winner === finalMatch.homeTeam}
                  <Check size={14} class="text-amber-400 shrink-0" />
                {/if}
              </button>
              ...
              <button
                ...
                disabled={away.isTba}
                class={`w-full flex items-center justify-between px-3 py-2 rounded-lg transition-all text-left ${
                  away.isTba ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-zinc-800/80'
                } ${finalMatch.winner === finalMatch.awayTeam ? 'bg-amber-500/20 text-amber-400 border border-amber-500 font-bold' : 'text-zinc-300'}`}
              >
                <div class="flex items-center gap-2 truncate">
                  {#if !away.isTba}
                    <span class="font-mono text-xs text-zinc-500 shrink-0">{away.id}</span>
      ...
      {#if state.knockoutBracket.matchesMap['M104']}
        {@const finalM = state.knockoutBracket.matchesMap['M104']}
        <div class="bg-zinc-950/80 border border-amber-900/30 rounded-xl p-4 flex flex-col items-center justify-center text-center space-y-2 py-6 bg-gradient-to-b from-black to-zinc-900">
          <Trophy class={`h-12 w-12 ${finalM?.winner ? 'text-amber-400 animate-bounce' : 'text-zinc-700'}`} />
          <div class="text-[10px] text-zinc-500 uppercase tracking-widest font-semibold">{t('champion')}</div>
          <div class="text-lg font-bold text-amber-400 min-h-[28px]">
            {#if finalM?.winner}
              {t(`teams.${finalM.winner}`)}
            {:else}
              <span class="text-zinc-600 text-xs italic font-normal">{t('tbd')}</span>
            {/if}
          </div>
        </div>
      {/if}
      ...
      {#if state.knockoutBracket.matchesMap['M103']}
        {@const thirdMatch = state.knockoutBracket.matchesMap['M103']}
        ...
        <div class="space-y-2">
          ...
          <div class="bg-zinc-950/80 border border-zinc-800/80 rounded-lg p-2.5 shadow-lg space-y-1.5 relative">
            <div class="text-[8px] font-mono text-zinc-500 absolute top-1 right-1.5">
              {thirdMatch.id}
            </div>
            ...
            <div class="space-y-1">
              <button
                ...
                class={`w-full flex items-center justify-between px-2 py-1 rounded transition-all text-left ${
                  home.isTba ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-zinc-800/50'
                } ${thirdMatch.winner === thirdMatch.homeTeam ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20 font-semibold' : 'text-zinc-300'}`}
              >
                <div class="flex items-center gap-1.5 truncate">
                  {#if !home.isTba}
                    <span class="font-mono text-xs text-zinc-500 shrink-0">{home.id}</span>
              ...
              <button
                ...
                class={`w-full flex items-center justify-between px-2 py-1 rounded transition-all text-left ${
                  away.isTba ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-zinc-800/50'
                } ${thirdMatch.winner === thirdMatch.awayTeam ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20 font-semibold' : 'text-zinc-300'}`}
              >
                <div class="flex items-center gap-1.5 truncate">
                  {#if !away.isTba}
                    <span class="font-mono text-xs text-zinc-500 shrink-0">{away.id}</span>

<script lang="ts">
  ...
  function t(key: string, vars?: Record<string, string | number>): string {
    ...
    const scope =
      (tools["bar-chart-generator"] as Record<string, unknown>) || {};
    ...
    if (typeof value !== "string")
      return `MISSING: tools.bar-chart-generator.${key}`;
  ...
  function parseCSV(csvText: string): { category: string; value: number }[] {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (cols.length >= 2) {
        result.push({ category: cols[0], value: Number(cols[1]) || 0 });
      }
    }
    return result;
  }
  ...
  function addRow() {
    ...
    data = [
      ...data,
      { id: newId, category: `${t("item")}${data.length + 1}`, value: 100 },
    ];
  }
  ...
  function loadSampleData() {
    ...
    data = [
      {
        id: `${baseId}-${idCounter}`,
        category: t("sampleData.productA"),
        value: 320,
      },
      {
        id: `${baseId}-${idCounter + 1}`,
        category: t("sampleData.productB"),
        value: 240,
      },
      {
        id: `${baseId}-${idCounter + 2}`,
        category: t("sampleData.productC"),
        value: 180,
      },
      {
        id: `${baseId}-${idCounter + 3}`,
        category: t("sampleData.productD"),
        value: 290,
      },
      {
        id: `${baseId}-${idCounter + 4}`,
        category: t("sampleData.productE"),
        value: 150,
      },
    ];
...
<div class="space-y-4">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        >
          <div>
            <label
              for="chart-title-input"
              class="block text-sm font-medium mb-1">{t("chartTitle")}</label
            >
            <input
    ...
    <!-- 右侧：图表预览 -->
    <div>
      <label for="chart-preview" class="block text-sm font-medium mb-2"
        >{t("chartPreview")}</label
      >
      <div
        id="chart-preview"

...
const chartTypeMap = {
  ...
  'ThemeRiverGenerator.svelte': 'theme-river-chart-generator',
  'CalendarHeatmapGenerator.svelte': 'calendar-heatmap-chart-generator',
};
...
chartComponents.forEach(file => {
  ...
  if (chartType) {
    ...
    // 5. 修复fontWeight类型不兼容错误 - 移除重复的as const
    content = content.replace(
      /fontWeight: 'bold' as const as const/g,
      "fontWeight: 'bold' as const"
    );
    content = content.replace(
      /fontWeight: 'normal' as const as const/g,
      "fontWeight: 'normal' as const"
    );
    
    // 6. 修复可访问性警告 - 为label添加for属性
    content = content.replace(
      /<label class="block text-sm font-medium mb-1">([^<]*)<\/label>/g,
      (match, labelText) => `<label for="label-${labelText}" class="block text-sm font-medium mb-1">${labelText}</label>`
    );

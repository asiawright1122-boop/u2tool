...
## TDK Cleanup Batch Evidence (v0.0.37)

The fifth candidate-driven cleanup batch trimmed exactly 10 synchronized
`root_base_match` overlong `seo_description` entries in `es`, `fr`, and `pt`.
...
- PASS
- `0` hard errors
- `2742` warning-only findings, down from `2752`
- `seo_description=2300`, down from `2310`
- `long=2736`, down from `2746`
- focused root/base sync check passed for all 10 edited pairs
...
- The candidate-export workflow continues to support small, auditable metadata
  cleanup batches.
- The next largest Latin-script candidates include `es/git-tag-manager`,
  `es/sql-query-optimizer`, `fr/json-to-protobuf-converter`,
  `fr/reading-time-calculator`, and `pt/css-to-tailwind`.

## TDK Cleanup Batch Evidence (v0.0.38)

The sixth candidate-driven cleanup batch trimmed exactly 10 synchronized
`root_base_match` overlong `seo_description` entries in `es`, `fr`, and `pt`.
...
- PASS
- `0` hard errors
- `2732` warning-only findings, down from `2742`
- `seo_description=2290`, down from `2300`
- `long=2726`, down from `2736`
- focused root/base sync check passed for all 10 edited pairs
...
- The candidate-export workflow continues to support small, auditable metadata
  cleanup batches.
- The next largest Latin-script candidates include `es/iban-validator`,
  `fr/markup-calculator`, `fr/project-estimation-calculator`,
  `fr/resource-allocation-planner`, `pt/git-branch-naming-validator`, and
  `pt/tip-calculator`.

## TDK Cleanup Batch Evidence (v0.0.39)

The seventh candidate-driven cleanup batch trimmed exactly 10 synchronized
`root_base_match` overlong `seo_description` entries in `es`, `fr`, and `pt`.
It intentionally skipped the two intervening high-overrun `ru` candidates for a
...
- PASS
- `0` hard errors
- `2722` warning-only findings, down from `2732`
- `seo_description=2280`, down from `2290`
- `long=2716`, down from `2726`
- focused root/base sync check passed for all 10 edited pairs
...
- The next largest Latin-script candidates include
  ...
  `pt/boxplot-chart-generator`, `pt/pace-calculator`, and
  `es/sql-to-mongodb-converter`.

## TDK Cleanup Batch Evidence (v0.0.40)

The eighth candidate-driven cleanup batch trimmed exactly 10 synchronized
`root_base_match` overlong `seo_description` entries in `es`, `fr`, and `pt`.
It intentionally skipped three intervening high-overrun `ru` candidates for a
later language-review batch.
...
- PASS
- `0` hard errors
- `2712` warning-only findings, down from `2722`
- `seo_description=2270`, down from `2280`
- `long=2706`, down from `2716`
- focused root/base sync check passed for all 10 edited pairs
...
- The candidate-export workflow continues to support small, auditable metadata
  cleanup batches.
- The next largest Latin-script candidates include `pt/placeholder-image`,

...
// 添加所有工具的翻译
tools.forEach(tool => {
  if (!enData.tools[tool.slug]) {
    enData.tools[tool.slug] = generateTranslation(tool.slug, tool.name);
    console.log(`✓ 添加 ${tool.slug} 翻译`);
  } else {
    console.log(`✓ ${tool.slug} 翻译已存在`);
  }
});

// 保存更新后的翻译文件
fs.writeFileSync(enFilePath, JSON.stringify(enData, null, 2) + '\n');
console.log(`\n✓ 成功添加 ${tools.length} 个工具的英文翻译`);

 ...
 * 
 * 使用方法:
 *   npx tsx scripts/batch54-siliconflow-translate.ts
 * 
 * 环境变量:
...
## 翻译质量要求
1. 母语级表达：翻译后的内容必须像母语者撰写的一样自然流畅
2. 避免翻译腔：不要逐字翻译，要理解原意后用目标语言重新表达
3. SEO 优化：${seoHints}
...
- 只翻译字符串值，不改变键名`;
  ...
  const userPrompt = `请将以下在线工具的内容翻译为 ${localeName}。只返回 JSON 对象：
  ...
  for (const locale of TARGET_LOCALES) {
    ...
    for (let i = 0; i < toolSlugs.length; i++) {
      ...
      if (existing?.detailed_description && existing?.usage_steps?.length > 0) {
        process.stdout.write(`  [${i + 1}/${toolSlugs.length}] ${slug}... ⏭️\n`);
        skipped++;
        continue;
      }

...
## 2026-05-09 Live Triage Notes

Representative live checks showed the highest-volume crawl-shape buckets are already handled coherently in production:

| Pattern | Representative URL | Live result | Triage |
|---|---|---|---|
| Legacy localized category | `https://www.u2tool.com/ar/tools/category/office` | `301` to `/ar/categories/office/` | Correct canonical consolidation. Wait for GSC recrawl. |
| Legacy unlocalized category | `https://www.u2tool.com/tools/category/office` | `301` to `/en/categories/office/` | Correct default-locale consolidation. |
| Legacy localized blog | `https://www.u2tool.com/ru/blog/password-security-best-practices` | `301` to `/ru/tools/password-generator/` | Correct targeted redirect for known legacy article slug. |
| Legacy pairwise compare | `https://www.u2tool.com/zh/compare/sql-to-mongo/json-to-csv` | `301` to `/zh/tools/sql-to-mongo/` | Correct targeted redirect for known pair. |
| Missing trailing slash | `https://www.u2tool.com/ko/tools/user-agent-parser` | `301` to `/ko/tools/user-agent-parser/` | Correct slash normalization. |
| Query-filtered tools index | `https://www.u2tool.com/zh/tools/?category=encoding` | `200`; canonical is `/zh/tools/` | Expected alternate canonical behavior. |
| Legacy Next.js static asset | `https://www.u2tool.com/_next/static/chunks/d96012bcfc98706a.js?dpl=...` | `410`; `X-Robots-Tag: noindex, nofollow` | Correct stale asset handling; do not redirect hashed assets to HTML. |

No broad redirect, noindex, canonical, or sitemap patch is recommended from Coverage drilldowns alone. The remaining high-volume canonical-shape tool URLs should be prioritized through Performance evidence.

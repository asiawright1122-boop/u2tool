...
equivalently. Classification first normalizes the final recognized head with
NFKC and whitespace removal, then requires the entire extracted head to match;
embedded suffixes cannot qualify. Normalized `NoSQL`, Japanese `非SQL`, and
Korean `비SQL` prefixes explicitly reject SQL precedence, including spaced and
full-width variants. The same executable heads without a positive SQL/query
prefix remain non-SQL.
...
Result: exit `0`; 7/7 files and 536/536 tests passed.
